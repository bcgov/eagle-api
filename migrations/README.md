...
deploy. They read the same `MONGODB_*` env vars as `run_migration.js`, are safe to re-run, and each
has `--help`.
...
writes lowercase goes out; until it has, those reports omit rows written earlier. `--dry-run` reports
which values and how many rows would change, and writes nothing.
