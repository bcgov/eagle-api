...
async function main() {
  ...
  try {
    ...
    const target_role_mappings = [];
    const target_roles = await goldKcAdminClient.roles.find({first:0,max:100});
    // goldKcAdminClient.users.addRealmRoleMappings()
    const max = 500;
    let start = 0;
    for (let i = 0; i < Math.ceil(total / max); i++) {
      ...
      for (let x = 0; x < userList.length; x++) {
        ...
        console.log('Target user',JSON.stringify(targetUser));
        if(Object.keys(targetUser).length > 0 && !target_role_mappings[targetUser['id']]) {
          const realm_role_mappings = await silverKcAdminClient.users.listRealmRoleMappings({ id: baseUser.id});
          const target_roles_mapping = realm_role_mappings.map(p=>{
            const target_role_map = target_roles.filter(q=>q.name === p.name)[0];
            if(target_role_map) return {id: target_role_map.id, name: target_role_map.name};
          })
          target_role_mappings.push({
            id: targetUser['id'],
            roles: target_roles_mapping
          });
          console.log('Target Roles', JSON.stringify(target_role_mappings));
        }
      }
    ...
    // mapping roles
    if(target_role_mappings.length > 0) {
      for(let i = 0; i < target_role_mappings.length; i++) {
        await goldKcAdminClient.users.addRealmRoleMappings({
          id: target_role_mappings[i].id,
          roles: target_role_mappings[i].roles
        })
      }
    }
